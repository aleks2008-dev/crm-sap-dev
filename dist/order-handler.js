...
async function default_1() {
    ...
    this.before('UPDATE', 'Orders', async (req) => {
        ...
        if (existing)
            req._previousStatus = existing.statusCode_code;
    });
    this.after('CREATE', 'Orders', async (data, req) => {
        if (data.IsActiveEntity === false)
            return;
        ...
        await logOrderInteraction(Interaction, order.customer_customerID, `New order created${amount}`, `Order ID: ${order.ID}`);
    });
    this.after('UPDATE', 'Orders', async (data, req) => {
        ...
        if (req._previousStatus !== undefined && req._previousStatus !== order.statusCode_code) {
            await logOrderInteraction(Interaction, order.customer_customerID, `Order status: ${order.statusCode_code}`, `Status changed from ${req._previousStatus} to ${order.statusCode_code}`);
        }
    });
    this.on('CREATE', 'OrderItems', async (req, next) => {
        ...
        await logOrderInteraction(Interaction, order.customer_customerID, 'Order item added', `Order ${orderID}: ${partLabel}, qty ${data.quantity ?? 0}, price ${data.price ?? 0}`);
        return item;
    });
    this.on('Orders_changeStatus', 'Orders', async (req) => {
        ...
        if (order.customer_customerID) {
            ...
            await logOrderInteraction(Interaction, order.customer_customerID, `Order status: ${newStatus}`, description);
        }
